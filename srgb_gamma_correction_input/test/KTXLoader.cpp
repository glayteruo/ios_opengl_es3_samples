...
KTXInfo KTXLoader::Load(const void* data, size_t dataSize, bool isSrgb)
{
	...
	for (auto i = 0u; i < header->numberOfMipmapLevels; ++i)
	{
		...
		if (header->glType == 0u)
		{
			auto format = (isSrgb) ? GL_COMPRESSED_SRGB8_ETC2 : GL_COMPRESSED_RGB8_ETC2;
			glCompressedTexImage2D(GL_TEXTURE_2D, i, format, width, height, 0, imageSize, image);
