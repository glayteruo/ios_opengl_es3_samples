...
enum
{
	...
	PASS2_UNIFROM_SHADOW_TEXTURE,
	PASS2_UNIFORM_HATCHING_TEXTURE,
	PASS2_UNIFORM_HATCHING_OFFSET,
    PASS2_NUM_UNIFORMS
};
...
@interface RMViewController () {
	...
	GLuint _hatchingTexture;
	GLuint _hatchingSampler;
	
	GLKVector2 _hatchingOffset;
}
...
- (void)setupGL
{
	...
	uniformsPass2[PASS2_UNIFROM_SHADOW_TEXTURE] = glGetUniformLocation(_programPass2, "shadowTexture");
	uniformsPass2[PASS2_UNIFORM_HATCHING_TEXTURE] = glGetUniformLocation(_programPass2, "hatchingTexture");
	uniformsPass2[PASS2_UNIFORM_HATCHING_OFFSET] = glGetUniformLocation(_programPass2, "hatchingOffset");
	...
	// ハッチング用テクスチャ読み込み
	NSData* texData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Textures/hatching" ofType:@"pvr"]];
	
	try
	{
		PVRInfo texInfo = PVRLoader::Load3D(texData.bytes, texData.length);
		_hatchingTexture = texInfo.name;
	}
	...
	// ハッチング用サンプラー作成
	glGenSamplers(1, &_hatchingSampler);
	glSamplerParameteri(_hatchingSampler, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glSamplerParameteri(_hatchingSampler, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glSamplerParameteri(_hatchingSampler, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
	glSamplerParameteri(_hatchingSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glSamplerParameteri(_hatchingSampler, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
}
...
- (void)tearDownGL
{
	...
	glDeleteSamplers(1, &_hatchingSampler);
	glDeleteTextures(1, &_hatchingTexture);
...
- (void)update
{
	...
	_hatchingOffset.x -= self.timeSinceLastUpdate * 0.05f;
	_hatchingOffset.y = _hatchingOffset.x * -0.5f;
}
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
	auto drawFunc = [&](int passNo, const DrawObject& obj, const GLKMatrix4& mvMat, const GLKMatrix4& shadowMat, GLenum mode, GLsizei count)
	{
		...
		else
		{
			...
			glUniform1i(uniformsPass2[PASS2_UNIFROM_SHADOW_TEXTURE], 0);
			
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_3D, _hatchingTexture);
			glBindSampler(1, _hatchingTexture);
			glUniform1i(uniformsPass2[PASS2_UNIFORM_HATCHING_TEXTURE], 1);
			
			glUniform2f(uniformsPass2[PASS2_UNIFORM_HATCHING_OFFSET], _hatchingOffset.x, _hatchingOffset.y);
		}
